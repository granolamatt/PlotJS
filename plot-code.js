...
//    element.onmousemove = this.formMouseMoved;
    var that = this;
    element.onmousemove = function(evt){that.doMouseMoved(evt)};
  	element.height = 600;
  	element.width = 800;
...
//    this.element = document.getElementById(id);
    ...
    console.log("New plot was made using canvas id " + 
    	id + " width " + this.width + " height " + this.height);
  ...
  getZoom() {
    var ret = new ZoomCoordinates(this.xAxisMin, this.xAxisMax, 
      this.yAxisMin, this.yAxisMax);
    return ret;
  }
  ...
  addData(data, color="#0000ff", autoscale=true, connectLines=true, tag="") {
...
//      ctx.stroke();
  ...
  drawBackground() {
    ...
    this.series.forEach(f => {
        f.drawPoints();
      });
    this.legendCtr = 0;
    this.drawSelection();
    this.backImage = ctx.getImageData(0, 0,
                             this.width,
                             this.height);
  }
